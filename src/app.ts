...
import { state, UI_TEXT, loadJson, restorePreferences, computeDailyQuoteVerseId, buildTranslationMap, hideLoading, showFatalError } from './state';
...
async function init() {
  try {
    const [chapters, verses, translations, languages, langData] = await Promise.all([
