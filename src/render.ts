...
export function runRevealForCurrentPanel() {
  const activePanel = dom.panels.find(
    (panel) => panel.dataset.section === state.currentSection,
  );
  if (!activePanel) {
    return;
  }
  const revealNodes = Array.from(
    activePanel.querySelectorAll<HTMLElement>(".reveal"),
  );
...
export function renderHomeSpotlight() {
  const dailyVerse =
    state.dailyQuoteVerseId !== null
      ? state.verseById.get(state.dailyQuoteVerseId)
      : undefined;
  if (!dailyVerse) {
...
export function renderVerses() {
  ...
  if (
    ...
    !visibleVerses.some(
      (verse) => getVerseAudioKey(verse) === state.activeAudioKey,
    )
  ) {
  ...
  if (!visibleVerses.length) {
    ...
    dom.versesList.innerHTML = visibleVerses
      .map(
        (verse) =>
          `<article class="verse-card reveal">${buildVerseHeaderMarkup(verse)}${buildVerseBodyMarkup(verse)}</article>`,
      )
      .join("");
  }
...
async function buildVerseImageCanvas(verse: any): Promise<HTMLCanvasElement> {
  const translation = getTranslation(verse.id, state.uiLanguage),
    verseLabel = `${t("chapterTag", { chapter: formatNumber(verse.chapter_number), verse: formatNumber(verse.verse_number) })}`,
    verseText = cleanText(verse.text),
    meaningText = translation?.text || t("translationMissing"),
    authorText = translation?.author
  ...
  const sanatanFont = "800 35px Cinzel, serif";
  ...
  const authorLines = authorText
    ? wrapText(ctx, authorText.trim(), innerWidth)
    : [];

  const lineHeightHeader = 36,
    lineHeightSanskrit = 42,
    lineHeightBody = 28;
  ...
  ctx.font = sanatanFont;
  ctx.fillText(sanatanHeader, margin + 40, y);
  let logo = new Image(35, 35);
  logo.src = "assets/logo.png";
  ctx.drawImage(logo, margin, y, 35, 35);
  y += 54;
...
export async function saveVerseAsImage(verse: any, message?: string) {
  try {
    ...
    link.click();
    alert(message ? message : "Downloaded successfully..");
...
export async function shareVerseAsImage(verse: any) {
  ...
  const file = new File(
    [blob],
    `gita-${verse.chapter_number}-${verse.verse_number}.png`,
    { type: "image/png" },
  );
  ...
  else await saveVerseAsImage(verse, "Sharing not supported, Downloaded image");
  return;
}
...
export function getFilteredVerses() {
  ...
  return state.verses.filter((verse) => {
    if (
      selectedChapter !== "all" &&
      String(verse.chapter_number) !== selectedChapter
    ) {
      return false;
    }
    ...
    const primaryTranslation =
      getTranslation(verse.id, state.uiLanguage)?.text || "";
    const secondaryTranslation =
      getTranslation(verse.id, secondary)?.text || "";
...
function translationBlockMarkup(
  language: string,
  entry: any,
  primary: boolean,
) {
